    ...
    test.describe(() => {
        ...
        test.describe(`E2E testing for ${name}`, () => {
            ...
            test("cannot edit widgets", async ({ page }) => {
                ...
                expect(
                    alertLocator.getByText(
                        "You are not allowed to edit widgets."
                    ),
                ...
                expect(newName, "should not have updated name").toEqual(
                    initialName
                );
                expect(
                    newDescription,
                    "should not have updated description"
                ).toEqual(initialDescription);
            });
        });
    });
});
